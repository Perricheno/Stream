...
test("new room starts host-less, source-less, paused at 0", () => {
  ...
  assert.deepEqual({ p: room.playback.isPlaying, s: room.playback.positionSeconds }, { p: false, s: 0 });
});

test("first member becomes host by userId; a later member does not", () => {
  const room = getOrCreateRoom(freshRoomId());
  addMember(room, member(1));
  assert.equal(room.hostUserId, 1);
  addMember(room, member(2));
  assert.equal(room.hostUserId, 1);
});

test("host survives a disconnect -> reconnect cycle (keyed by userId, not socket)", () => {
  const room = getOrCreateRoom(freshRoomId());
  addMember(room, member(1, "sock-a"));
  addMember(room, member(2, "sock-b"));

  removeMember(room, "sock-a"); // host's socket drops
  assert.equal(room.hostUserId, 1, "removeMember must NOT reassign host");

  addMember(room, member(1, "sock-a2")); // host reconnects with a fresh socket
  assert.equal(room.hostUserId, 1);
  assert.equal(room.members.find((m) => m.userId === 1)?.socketId, "sock-a2");
});

test("removeMember never promotes; promoteNextHost hands the role to the next present member", () => {
  const room = getOrCreateRoom(freshRoomId());
  addMember(room, member(1));
  addMember(room, member(2));
  room.playback = { isPlaying: true, positionSeconds: 12.5, updatedAtServerTime: 1000 };

  removeMember(room, room.members.find((m) => m.userId === 1)!.socketId);
  assert.equal(room.hostUserId, 1, "still nominally the host until the grace elapses");

  const newHost = promoteNextHost(room);
  assert.equal(newHost, 2);
  assert.equal(room.hostUserId, 2);
  assert.deepEqual(room.playback, { isPlaying: true, positionSeconds: 12.5, updatedAtServerTime: 1000 }, "playback untouched");
});

test("promoteNextHost with nobody left clears the host", () => {
  const room = getOrCreateRoom(freshRoomId());
  addMember(room, member(1, "s1"));
  removeMember(room, "s1");
  assert.equal(promoteNextHost(room), null);
  assert.equal(room.hostUserId, null);
});

test("addMember cancels a pending disconnect timer for that user", () => {
  mock.timers.enable({ apis: ["setTimeout"] });
  const room = getOrCreateRoom(freshRoomId());
  addMember(room, member(1, "s1"));

  let fired = false;
  room.disconnectTimers.set(1, setTimeout(() => (fired = true), 10_000));
  removeMember(room, "s1");

  addMember(room, member(1, "s1-again")); // reconnect within grace
  assert.equal(room.disconnectTimers.has(1), false, "timer removed from the map");
  mock.timers.tick(30_000);
  assert.equal(fired, false, "the pending 'they really left' callback never ran");
});
